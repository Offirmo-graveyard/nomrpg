...
#include <iomanip>
	...
	// http://stackoverflow.com/a/3233069/587407
...
typedef int_fast64_t DEC_INT64;
#else
...
//      if ((abs(value1) <= INT32_MAX) || (abs(value2) <= INT32_MAX))
      if ((abs(value1) <= std::numeric_limits<int32_t>::max()) || (abs(value2) <= std::numeric_limits<int32_t>::max()))
...
template <int prec>
std::ostream& operator<<(std::ostream& os, const decimal<prec> & d)
{
  std::string helper;
  os << toString(d, helper);
  return os;
